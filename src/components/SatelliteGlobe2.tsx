...
const SatelliteGlobe: React.FC<SatelliteGlobeProps> = ({
  ...
  const renderMarkers = () => {
    ...
    return markers.map(marker => {
      const { x, y, z } = latLngToXYZ(marker.latitude, marker.longitude);
      ...
      return (
        <div
          ...
          style={{
            ...
            boxShadow: `0 0 ${isSelected ? '20px' : '10px'} rgba(0, 119, 182, 0.3)`,
  ...
  const styles = {
    ...
    globe: {
      ...
      background: viewMode === 'satellite' 
        ? 'radial-gradient(circle at 30% 30%, #2E8B57, #228B22, #006400)'
        : 'radial-gradient(circle at 30% 30%, #4CAF50, #388E3C, #2E7D32)',
      position: 'relative' as const,
      transform: `rotateY(${rotation}deg) rotateX(10deg)`,
      boxShadow: `
        inset -20px -20px 50px rgba(0, 0, 0, 0.3),
        0 0 50px rgba(0, 119, 182, 0.2)
      `,
      transformStyle: 'preserve-3d' as const,
  ...
  return (
    <div style={styles.container}>
      ...
      <style>{`
        ...
        .marker:hover {
          transform: translate3d(${0}px, ${0}px, ${0}px) translate(-50%, -50%) scale(1.2) !important;
        }
      `}</style>
    </div>
  );
